...
| oci://ghcr.io/clickhouse | clickhouseOperator(clickhouse-operator-helm) | 0.0.4 |

## Values

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| clickhouse | object | `{"auth":{"existingSecret":"","existingSecretKey":"","password":"","username":"default"},"cluster":{"affinity":{},"image":{"repository":"clickhouse/clickhouse-server","tag":"24.8"},"nodeSelector":{},"profileSettings":{},"replicas":1,"resources":{},"settings":{},"storage":{"accessModes":["ReadWriteOnce"],"className":"","size":"100Gi"},"tolerations":[]},"clusterEnabled":true,"database":"default","deploy":true,"host":"","httpPort":8123,"keeper":{"affinity":{},"enabled":true,"image":{"repository":"clickhouse/clickhouse-keeper","tag":"24.8"},"nodeSelector":{},"replicas":3,"resources":{},"storage":{"accessModes":["ReadWriteOnce"],"className":"","size":"10Gi"},"tolerations":[]},"migration":{"autoMigrate":true,"ssl":false,"url":""},"nativePort":9000}` | release notes before upgrading and pin the operator chart version explicitly. |
...
| clickhouse.cluster | object | `{"affinity":{},"image":{"repository":"clickhouse/clickhouse-server","tag":"24.8"},"nodeSelector":{},"profileSettings":{},"replicas":1,"resources":{},"settings":{},"storage":{"accessModes":["ReadWriteOnce"],"className":"","size":"100Gi"},"tolerations":[]}` | ------------------------------------------------------------------------- |
...
| clickhouse.cluster.replicas | int | `1` | automatically forces `CLICKHOUSE_CLUSTER_ENABLED=false`). 2+ requires Keeper enabled. |
...
| clickhouse.clusterEnabled | bool | `true` | When `clickhouse.deploy: true` AND `cluster.replicas: 1`, this is ignored and forced to false. |
...
| clickhouseOperator | object | `{"certManager":{"enable":true,"install":true},"crd":{"enable":true,"keep":true},"deploy":true,"manager":{"image":{"pullPolicy":"IfNotPresent","repository":"ghcr.io/clickhouse/clickhouse-operator"},"replicas":1,"resources":{}},"metrics":{"enable":true},"pdbManagement":{"enable":true},"webhook":{"enable":true}}` | operator's sub-chart and gated on `certManager.install` below. |
| clickhouseOperator.certManager.enable | bool | `true` | Tell the operator to use cert-manager for its admission webhook. |
| clickhouseOperator.certManager.install | bool | `true` | is already installed in the cluster. |
| clickhouseOperator.crd.enable | bool | `true` | Install the operator CRDs. |
| clickhouseOperator.crd.keep | bool | `true` | Keep CRDs on `helm uninstall`. Recommended to avoid losing CRs accidentally. |
| clickhouseOperator.deploy | bool | `true` | The operator is cluster-scoped — only ONE instance should run per cluster. |
| clickhouseOperator.manager | object | `{"image":{"pullPolicy":"IfNotPresent","repository":"ghcr.io/clickhouse/clickhouse-operator"},"replicas":1,"resources":{}}` | ------------------------------------------------------------------------- |
| extraManifests | list | `[]` |  |
...
| postgresql.image | object | `{"pullPolicy":"IfNotPresent","repository":"postgres","tag":"18-alpine"}` | ------------------------------------------------------------------------- |
